...
基于 Vite + React 19 + TypeScript 6 的倒计时页面，支持到达目标时间后自动切换为正计时，集成天气显示与时间感知背景。
...
所有变量以 `VITE_` 开头，构建时注入。本地开发复制 `destination/.env.example` 为 `destination/.env`。
...
需要 Node.js 18+。`.node-version` 指定 Vercel 使用的版本。

## Vercel 部署

### 仓库结构注意

项目代码在 `destination/` 子目录中，**部署前必须先设置 Root Directory**。
...
- 点击 **Save**
- ⚠️ 不设置此项会导致部署失败：`ENOENT package.json`
...
也可在 `destination/.env` 中设置，构建时自动读取。

**4. 配置域名（可选）**

- **Settings → Domains**
- 添加自定义域名（如 `time.webspacex.eu.org`）
- DNS 中添加 CNAME 记录指向 `cname.vercel-dns.com`

**5. 部署**

- 推送到 `master` 分支自动触发部署
- 也可在 **Deployments** 页面手动 **Redeploy**
